	...
	for x in range(grid_size.x-1):
		for y in range(grid_size.y-1):
			for z in range(grid_size.z-1):
				# This used to be a function call, let's see if moving it here is cheaper
				# Get cube indicex
				var cube_indices = [
					int((x)+((z+1)*grid_size.x)+((y)*grid_slice_size)),
					int((x+1)+((z+1)*grid_size.x)+((y)*grid_slice_size)),
					int((x+1)+((z)*grid_size.x)+((y)*grid_slice_size)),
					int((x)+((z)*grid_size.x)+((y)*grid_slice_size)),
					int((x)+((z+1)*grid_size.x)+((y+1)*grid_slice_size)),
					int((x+1)+((z+1)*grid_size.x)+((y+1)*grid_slice_size)),
					int((x+1)+((z)*grid_size.x)+((y+1)*grid_slice_size)),
					int((x)+((z)*grid_size.x)+((y+1)*grid_slice_size)),
				]
